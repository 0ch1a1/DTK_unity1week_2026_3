...
public class Ochiai_EnemyMove_Script : MonoBehaviour
{
    ...
    private void MoveManager()      //状況に応じて_currentMoveStateを切り替える関数
    {
        switch (_currentMoveState)
        {
            case MovingState.Wait:  //指定の時間後に現在のstateをnextMoveStateのstateに変える
                if (!isDelayed)
                {
                    isDelayed = true;
                ...
                break;
            case MovingState.Chase: //追っている目標に追いつくとreturnのstateに変える
                ...
                break;
            case MovingState.Return:  //巡回場所に着くとpatrolのstateに切り替える
